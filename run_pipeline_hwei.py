#!/usr/bin/env python3
"""
Hwei B站录播中单索引 — 统一入口程序
=========================================
用法:
  python run_pipeline_hwei.py --bv BV1xKVC6NEw2          # 处理指定视频
  python run_pipeline_hwei.py --latest 3                  # 处理UP主最新3个长录播

流程:
  环境检查 → cookies验证 → 720p确认 → 下载 → 抽帧 →
  粗筛 → OpenCLIP识别 → Hwei判定 → 合并投票 → Excel导出
"""
import argparse, json, os, sys, time, subprocess
from datetime import datetime

CONFIG_FILE = "config_hwei.json"
CHAMPION_NAME = "Hwei"
CHAMPION_KEY = "hwei"
...
def load_config():
    ...
    if not os.path.isfile(config_path):
        fatal(f"{CONFIG_FILE} not found at {config_path}")
    with open(config_path, "r", encoding="utf-8") as f:
        return json.load(f)


def get_cookies_path(cli_path=None):
    """Get cookies.txt path from CLI arg or interactive prompt."""
    if cli_path:
...
def get_output_xlsx_path(bv_id, config):
    ...
    return os.path.join(out_dir, f"{CHAMPION_KEY}_mid_index_{bv_id}.xlsx")
...
def step_discover_videos(args, config, cookies_path=None):
    ...
    if args.bv:
        ...
        # Latest N mode — uses mobile API with title+duration in one pass
        n = args.latest
        mid = config.get("up主", {}).get("mid", 1417418056)
        log(f"  获取UP主 mid={mid} 最新视频（低频分页，遇412即停）...")

        from scripts.bilibili_utils import fetch_and_filter_videos, get_video_info
        # Fetch enough pages to find N matches (keyword match rate is low for this UP主)
        candidates = fetch_and_filter_videos(mid, config, max_videos=200, cookies_path=cookies_path)
        if not candidates:
            fatal("未找到符合条件的长录播。请检查标题关键词和时长配置。")

        log(f"  标题匹配: {len(candidates)} 个")
...
def step_pipeline(bv_id, config):
    """Run the main pipeline (CLIP → Hwei → Excel)."""
    log(f"  识别: {bv_id}")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, CONFIG_FILE)
    result = subprocess.run(
        [sys.executable, "scripts/pipeline_video.py", "--config", config_path, bv_id],
        capture_output=True, text=True, encoding="utf-8", errors="replace",
        timeout=7200,
    )
    stdout = result.stdout or ""
    ...
    log(f"    结果: {games} 局{CHAMPION_NAME}, {low} 低置信度")
    return games, low
...
def step_generate_report(results, config, skipped=0):
    ...
    for i, r in enumerate(results):
        ...
        lines.append(f"    {CHAMPION_NAME}对局: {r.get('games', '?')} | 低置信度: {r.get('low_conf', '?')}")
...
def main():
    parser = argparse.ArgumentParser(
        description=f"{CHAMPION_NAME} B站录播中单索引 — 一键式自动化工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python run_pipeline_hwei.py --bv BV1xKVC6NEw2
  python run_pipeline_hwei.py --latest 3
  python run_pipeline_hwei.py --bv BV1xKVC6NEw2 --skip-download  # 跳过下载(已下载时)
        """,
    )
    parser.add_argument("--bv", type=str, help="指定单个BV号处理")
    parser.add_argument("--latest", type=int, help=f"处理UP主最新N个长录播（标题含关键词）")
    ...
    parser.add_argument("--cookies-path", type=str, default=None,
                        help="cookies.txt 路径 (非交互模式)")
    ...
    # 2. Cookies
    cookies_path = get_cookies_path(args.cookies_path)
    ...
    for i, v in enumerate(videos):
        ...
        try:
            ...
            results.append({
                ...
                "games": games,
                "low_conf": low,
                "xlsx": f"output/{CHAMPION_KEY}_mid_index_{bv}.xlsx",
                "notes": "",
            })
    ...
    if results:
        ...
        merge_result = subprocess.run(
            [sys.executable, "scripts/merge_excel.py", "--prefix", CHAMPION_KEY],
            capture_output=True, text=True, encoding="utf-8", errors="replace",
            timeout=120,
        )
    ...
    summary = f"全部完成! 处理 {len(videos)} 个视频, {total_games} 局{CHAMPION_NAME}对局"
    if skipped:
